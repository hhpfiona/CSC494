...
class VllmEngine(BaseEngine):
    ...
    async def _generate(
        ...
        **input_kwargs,
        ...
        if system:
            use_messages = [{"role": "system", "content": system}] + messages
        else:
            use_messages = messages
        print(use_messages)
        prompt_ids = self.tokenizer.apply_chat_template(use_messages, tools=tools, chat_template=self.chat_template,
    ...
    async def chat(
        ...
        **input_kwargs,
        ...
        prompt = self.tokenizer.decode(final_output.prompt_token_ids, skip_special_tokens=False,
                                       clean_up_tokenization_spaces=True)
        print(f"\033[94m{prompt}\033[0m")
        res = self.tokenizer.decode(final_output.outputs[0].token_ids, skip_special_tokens=True,
                                    clean_up_tokenization_spaces=True)
        print(f"\033[91m{res}\033[0m")
        results = []
        for output in final_output.outputs:
            # print(f'output_text: {output.text}')
            results.append(
                Response(
